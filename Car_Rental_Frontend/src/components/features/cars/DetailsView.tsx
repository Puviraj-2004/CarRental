...
export const DetailsView = ({ car, loading, onClose, onBook, t }: DetailsViewProps) => {
  if (loading) {
    return (
  ...
  const pricingOptions = [
    { label: t('cars.perHour'), value: car.pricePerHour, icon: <AccessTimeIcon fontSize="small" /> },
    { label: t('carDetails.perDay'), value: car.pricePerDay, icon: <EventIcon fontSize="small" /> },
    { label: t('cars.perKm'), value: car.pricePerKm, icon: <SpeedIcon fontSize="small" /> },
      ...
      <Container maxWidth="lg">
        ...
        <Grid container spacing={5}>
          ...
          <Grid item xs={12} md={5}>
            <Stack spacing={4}>
              <Box>
                <Typography variant="h6" fontWeight={800} mb={2} color="#0F172A">{t('cars.technicalSpecs')}</Typography>
                <Grid container spacing={2}>
                  {[
                    { icon: <SettingsIcon />, label: car.transmission },
                    { icon: <FuelIcon />, label: car.fuelType },
                    { icon: <SeatIcon />, label: `${car.seats} ${t('cars.seats')}` },
                    { icon: <SpeedIcon />, label: `${car.currentOdometer || 0} ${t('common.km')}` },
                  ].map((spec, i) => (
              ...
              <Box>
                <Typography variant="h6" fontWeight={800} mb={2} color="#0F172A">{t('cars.rentalPricing')}</Typography>
                <Stack spacing={2}>
              ...
              <Alert icon={<InfoIcon />} severity="info" sx={{ borderRadius: 3, bgcolor: '#EFF6FF', border: '1px solid #DBEAFE', color: '#1E40AF' }}>
                {t('cars.refundableDeposit')}: <strong>€{car.depositAmount?.toFixed(2)}</strong>
              </Alert>
          ...
          <Grid item xs={12}>
            <Paper elevation={0} sx={{ p: 4, borderRadius: 5, border: '1px solid #E2E8F0' }}>
              <Typography variant="h6" fontWeight={800} mb={2}>{t('cars.aboutVehicle')}</Typography>
              <Typography variant="body1" color="text.secondary" sx={{ lineHeight: 1.8, maxWidth: '800px' }}>
                {car.descriptionEn || t('cars.defaultDescription')}
              </Typography>
            </Paper>
          </Grid>
        </Grid>
