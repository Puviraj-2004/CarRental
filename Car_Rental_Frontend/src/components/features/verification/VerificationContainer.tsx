...
export const VerificationContainer = ({ token }: { token: string }) => {
  ...
  const handleFileUpload = async (file: File, type: string, side?: string) => {
    ...
    try {
      ...
      if (res?.isQuotaExceeded) {
        setAlert({ 
          open: true, 
          message: t('verificationFlow.aiLimitReached'), 
          severity: 'warning' 
        });
        return;
      }
      ...
      if (res?.fallbackUsed) {
        setAlert({ 
          open: true, 
          message: t('verificationFlow.photoBlurry'), 
          severity: 'info' 
        });
      }
      ...
      setAlert({ 
        open: true, 
        message: t('verificationFlow.ocrFailed'), 
        severity: 'error' 
      });
  ...
  const handleNext = () => {
    ...
    if (currentStep === 0) {
      ...
      if (!licenseData.expiry || !licenseData.issueDate || !licenseData.dob) {
        setAlert({ 
          open: true, 
          message: t('verificationFlow.fillLicenseDates'), 
          severity: 'error' 
        });
        return;
      }
      ...
      if (returnDate && expiry <= new Date(returnDate)) {
        setAlert({ 
          open: true, 
          message: t('verificationFlow.licenseExpiredBeforeReturn'), 
          severity: 'error' 
        });
        return;
      }
      ...
      if (age < 21) {
        setAlert({ 
          open: true, 
          message: t('verificationFlow.minimumAge'), 
          severity: 'error' 
        });
        return;
      }
      ...
      if (experienceYears < 2) {
        setAlert({ 
          open: true, 
          message: t('verificationFlow.minimumExperience'), 
          severity: 'error' 
        });
        return;
      }
      ...
      if (!hasValidCategory) {
        setAlert({ 
          open: true, 
          message: t('verificationFlow.licenseCategoryMismatch', { category: requiredCategory }), 
          severity: 'error' 
        });
        return;
      }
      ...
      if (!cniData.name || !cniData.number || !cniData.dob || !cniData.idExpiry) {
        setAlert({ 
          open: true, 
          message: t('verificationFlow.fillIdFields'), 
          severity: 'error' 
        });
        return;
      }
      ...
      if (!cniNameNorm.includes(licenseNameNorm) && !licenseNameNorm.includes(cniNameNorm)) {
        if(!confirm(t('verificationFlow.nameIdMismatch'))) {
            return;
        }
      }
      ...
      if (cniData.dob !== licenseData.dob) {
         if(!confirm(t('verificationFlow.dobMismatch', { licenseDob: licenseData.dob, idDob: cniData.dob }))) {
            return;
         }
      }
  ...
  try {
    ...
    setAlert({ 
      open: true, 
      message: t('verificationFlow.submissionFailed'), 
      severity: 'error' 
    });
  }
};
