...
export const FooterView = ({ settings, loading, t }: FooterViewProps) => {
  if (loading) {
    return (
      ...
      <Container maxWidth="xl">
        <Grid container spacing={5}>
          ...
          <Grid item xs={12} md={4} sx={{ textAlign: { xs: 'center', md: 'left' } }}>
            ...
            <Typography variant="body2" sx={{ opacity: 0.7, lineHeight: 1.8, mb: 4, maxWidth: { md: '300px' }, mx: { xs: 'auto', md: 0 } }}>
              {settings.description || t('footer.defaultDescription')}
            </Typography>
          ...
          {/* 2. QUICK LINKS */}
          <Grid item xs={6} md={2}>
            <Typography variant="subtitle1" sx={{ fontWeight: 700, mb: 3 }}>{t('footer.company')}</Typography>
            <Stack spacing={1.5}>
              <MuiLink component={Link} href="/about" underline="none" color="inherit" sx={{ opacity: 0.7, '&:hover': { opacity: 1, color: '#60A5FA' } }}>{t('footer.aboutUs')}</MuiLink>
              <MuiLink component={Link} href="/cars" underline="none" color="inherit" sx={{ opacity: 0.7, '&:hover': { opacity: 1, color: '#60A5FA' } }}>{t('footer.ourFleet')}</MuiLink>
            </Stack>
          </Grid>

          {/* 3. SUPPORT */}
          <Grid item xs={6} md={2}>
            <Typography variant="subtitle1" sx={{ fontWeight: 700, mb: 3 }}>{t('footer.support')}</Typography>
            <Stack spacing={1.5}>
              <MuiLink href="#" underline="none" color="inherit" sx={{ opacity: 0.7, '&:hover': { opacity: 1, color: '#60A5FA' } }}>{t('footer.helpCenter')}</MuiLink>
            </Stack>
          </Grid>

          {/* 4. CONTACT INFO */}
          <Grid item xs={12} md={4}>
            <Typography variant="subtitle1" sx={{ fontWeight: 700, mb: 3, textAlign: { xs: 'center', md: 'left' } }}>{t('footer.contactUs')}</Typography>
            <Stack spacing={2.5}>
        ...
        >
          <Typography variant="caption" sx={{ opacity: 0.5 }}>
            © {new Date().getFullYear()} {settings.companyName}. {t('footer.allRightsReserved')}
          </Typography>
          <Typography variant="caption" sx={{ opacity: 0.3 }}>
            {t('footer.designedForExcellence')}
          </Typography>
        </Stack>
      </Container>
    </Box>
