 ...
 */
const CarImage = ({ src, alt, size = 'medium', noImageText = 'No Image' }: { src?: string; alt: string; size?: 'small' | 'medium' | 'large'; noImageText?: string }) => {
    ...
    >
      ...
      {error ? (
        <Box sx={{ 
          ...
          <Typography variant="caption" sx={{ fontSize: '0.65rem', fontWeight: 600 }}>{noImageText}</Typography>
        </Box>
      ) : (
        <img
          src={src}
...
export const AdminCarsView = ({
  ...
  deleteDialogOpen, setDeleteDialogOpen, confirmDelete, loading, onCarSelect, t
}: any) => {
  ...
  return (
    <Box sx={{ pb: 5 }}>
      ...
      >
        <Box>
          <Typography variant="h4" fontWeight={900} color="#0F172A">{t('admin.fleetManagement')}</Typography>
          <Typography variant="body2" color="text.secondary">
            {t('admin.professionalVehicles', { count: String(cars.length) })}
          </Typography>
        </Box>
        ...
        <Grid container spacing={2} alignItems="center">
          <Grid item xs={12} sm={6} md={2.5}>
            <Autocomplete
              ...
              renderInput={(params) => (
                <TextField {...params} label={t('admin.brand')} placeholder={t('admin.allBrands')} />
              )}
            />
          </Grid>
          ...
          <Grid item xs={12} sm={6} md={2.5}>
            <Autocomplete
              ...
              renderInput={(params) => (
                <TextField {...params} label={t('admin.model')} placeholder={t('admin.allModels')} />
              )}
            />
          </Grid>
          ...
          {cars.map((car: any) => (
            <Grid item xs={12} sm={6} lg={4} key={car.id}>
              <Card elevation={0} sx={{ borderRadius: 4, border: '1px solid #E2E8F0', overflow: 'hidden', height: '100%', display: 'flex', flexDirection: 'column', transition: '0.3s', '&:hover': { transform: 'translateY(-4px)', boxShadow: '0 12px 24px -10px rgba(0,0,0,0.1)' } }}>
                <Box sx={{ p: 1.5, pb: 0 }}>
                  {/* Car Image Box */}
                  <CarImage src={car.images?.[0]?.url} alt={`${car.brand.name} ${car.model.name}`} size="medium" noImageText={t('admin.noImage')} />
                  <Chip 
                    label={formatEnum(car.status)} 
                    size="small" 
                    sx={{ 
                ...
                <CardContent sx={{ p: 3, pt: 2, flexGrow: 1 }}>
                  ...
                  <Stack direction="row" spacing={1} mb={3}>
                    ...
                    <Chip label={formatEnum(car.fuelType)} size="small" variant="outlined" sx={{ fontWeight: 600, borderRadius: 1.5 }} />
                  </Stack>
                  <Divider sx={{ mb: 2, borderStyle: 'dashed' }} />
                  <Stack direction="row" justifyContent="space-between" alignItems="center">
                    <Typography variant="h5" fontWeight={900}>€{car.pricePerDay}<Typography component="span" variant="caption" color="text.secondary" sx={{ ml: 0.5 }}>{t('adminCars.perDay')}</Typography></Typography>
          ...
          <Table>
            <TableHead sx={{ bgcolor: '#F8FAFC' }}>
              <TableRow>
                ...
                <TableCell sx={{ fontWeight: 800, color: '#64748B' }}>{t('admin.specifications')}</TableCell>
                <TableCell sx={{ fontWeight: 800, color: '#64748B' }}>{t('admin.dailyRate')}</TableCell>
                <TableCell sx={{ fontWeight: 800, color: '#64748B' }}>{t('admin.status')}</TableCell>
                <TableCell align="right" sx={{ fontWeight: 800, color: '#64748B' }}>{t('admin.actions')}</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {cars.map((car: any) => (
                <TableRow key={car.id} hover>
                  <TableCell>
                    <Stack direction="row" spacing={2} alignItems="center">
                      <CarImage src={car.images?.[0]?.url} alt={`${car.brand.name} ${car.model.name}`} size="small" noImageText={t('admin.noImage')} />
                      <Box>
                  ...
                  <TableCell>
                    <Typography variant="body2" color="#334155" fontWeight={500}>{car.year} • {formatEnum(car.fuelType)} • {formatEnum(car.transmission)}</Typography>
                  </TableCell>
                  <TableCell><Typography fontWeight={900} color="#0F172A">€{car.pricePerDay?.toFixed(2)}</Typography></TableCell>
                  <TableCell>
                    <Chip label={formatEnum(car.status)} size="small" sx={{ fontWeight: 800, fontSize: '0.65rem', bgcolor: car.status === 'AVAILABLE' ? '#ECFDF5' : '#FEF2F2', color: car.status === 'AVAILABLE' ? '#047857' : '#B91C1C' }} />
                  </TableCell>
      ...
      <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)} PaperProps={{ sx: { borderRadius: 4, p: 1 } }}>
        <DialogTitle sx={{ fontWeight: 900, textAlign: 'center', pt: 3 }}>{t('admin.removeVehicle')}</DialogTitle>
        <Box sx={{ px: 4, pb: 2, textAlign: 'center' }}>
          <Typography variant="body2" color="text.secondary">{t('admin.deleteVehicleConfirm')}</Typography>
        </Box>
        <DialogActions sx={{ p: 3, justifyContent: 'center', gap: 2 }}>
          <Button onClick={() => setDeleteDialogOpen(false)} sx={{ fontWeight: 700, color: '#64748B' }}>{t('admin.cancel')}</Button>
          <Button variant="contained" color="error" onClick={confirmDelete} sx={{ fontWeight: 700, borderRadius: 2, px: 3, bgcolor: '#EF4444' }}>{t('admin.yesDelete')}</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );
