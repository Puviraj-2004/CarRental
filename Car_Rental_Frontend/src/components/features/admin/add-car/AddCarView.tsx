      ...
      <Box sx={{ flex: 1, overflowY: 'auto', p: { xs: 2, sm: 4 } }}>
        {activeTab === 0 && (
          <Grid container spacing={2}>
            ...
            <Grid item xs={6} md={3}><TextField fullWidth label={t('admin.year')} name="year" type="number" size="small" required value={formData.year} onChange={onInputChange} /></Grid>
            <Grid item xs={6} md={3}>
            ...
            <Grid item xs={12} md={6}><TextField fullWidth label={t('admin.plateNumber')} name="plateNumber" size="small" required value={formData.plateNumber} onChange={onInputChange} /></Grid>
            <Grid item xs={6} md={3}>
            ...
            <Grid item xs={6} md={3}><TextField fullWidth label={t('admin.seats')} name="seats" type="number" size="small" required value={formData.seats} onChange={onInputChange} /></Grid>
            <Grid item xs={6} md={3}>
              <FormControl fullWidth size="small" required>
        ...
        {activeTab === 1 && (
          <Grid container spacing={3}>
            <Grid item xs={12}><Typography variant="subtitle2" sx={{ fontWeight: 600, color: '#293D91' }}>💰 {t('admin.pricingDeposits')}</Typography></Grid>
            <Grid item xs={12} md={6}><TextField fullWidth size="small" label={t('admin.pricePerDay')} name="pricePerDay" type="number" required value={formData.pricePerDay || ''} onChange={onInputChange} InputProps={{ startAdornment: '€' }} /></Grid>
            <Grid item xs={12} md={6}><TextField fullWidth size="small" label={t('admin.securityDeposit')} name="depositAmount" type="number" value={formData.depositAmount || ''} onChange={onInputChange} InputProps={{ startAdornment: '€' }} /></Grid>
            <Grid item xs={12}><Typography variant="subtitle2" sx={{ fontWeight: 600, color: '#293D91' }}>🚗 {t('admin.kmLimitsTracking')}</Typography></Grid>
            <Grid item xs={12} md={6}><TextField fullWidth size="small" label={t('admin.dailyKmLimit')} name="dailyKmLimit" type="number" value={formData.dailyKmLimit || ''} onChange={onInputChange} InputProps={{ endAdornment: formData.dailyKmLimit ? t('common.kmPerDay') : null }} /></Grid>
            <Grid item xs={12} md={6}><TextField fullWidth size="small" label={t('admin.extraKmCharge')} name="extraKmCharge" type="number" required value={formData.extraKmCharge || ''} onChange={onInputChange} InputProps={{ startAdornment: '€' }} /></Grid>
            <Grid item xs={12}><TextField fullWidth size="small" label={t('admin.currentOdometer')} name="currentOdometer" type="number" required value={formData.currentOdometer || ''} onChange={onInputChange} /></Grid>
          </Grid>
        )}
              ...
              {imagePreviews.map((p: string, i: number) => (
                <Grid item xs={3} sm={2} key={i}>
                  <Box sx={{ position: 'relative', height: 80, borderRadius: 2, overflow: 'hidden', border: i === primaryImageIndex ? '2px solid #293D91' : '1px solid #E2E8F0', cursor: 'pointer' }} onClick={() => setPrimaryImageIndex(i)}>
                    ...
                    {i === primaryImageIndex && <Box sx={{ position: 'absolute', bottom: 0, width: '100%', bgcolor: '#293D91', color: 'white', fontSize: '10px', textAlign: 'center' }}>{t('admin.main')}</Box>}
