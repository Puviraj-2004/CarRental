  ...
  return (
    <Paper elevation={0} sx={{ 
      ...
      <Box sx={{ p: { xs: 3, md: 5 }, minHeight: 450 }}>
        {activeTab === 0 && (
          <Grid container spacing={3}>
            ...
            <Grid item xs={6} md={3}>
              <TextField fullWidth label={t('admin.seats')} name="seats" type="number" size="small" value={formData.seats} onChange={onInputChange} />
            </Grid>
        ...
        {activeTab === 1 && (
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} md={6}>
              <TextField fullWidth size="small" label={t('admin.dailyKmLimit')} name="dailyKmLimit" type="number" value={formData.dailyKmLimit} onChange={onInputChange} />
            </Grid>
            <Grid item xs={12} md={6}>
              <TextField fullWidth size="small" label={t('admin.extraKmCharge')} name="extraKmCharge" type="number" value={formData.extraKmCharge} onChange={onInputChange} />
            </Grid>
            <Grid item xs={12}>
              <TextField fullWidth size="small" label={t('admin.currentOdometer')} name="currentOdometer" type="number" value={formData.currentOdometer} onChange={onInputChange} />
            </Grid>
          </Grid>
        )}
        ...
        {activeTab === 2 && (
          <Box>
             <Box component="label" sx={{ 
              ...
              <Typography variant="body2" fontWeight={600} color="#64748B">{t('admin.clickToUpload')}</Typography>
            </Box>
              ...
              {existingImages.map((img: any) => (
                <Grid item xs={4} sm={3} md={2} key={img.id}>
                  <Box sx={{ 
                    ...
                    {img.isPrimary && <Chip label={t('admin.main')} size="small" sx={{ position: 'absolute', bottom: 4, left: '50%', transform: 'translateX(-50%)', height: 16, fontSize: 9, bgcolor: '#0F172A', color: 'white' }} />}
      ...
      <Stack direction="row" justifyContent="flex-end" spacing={2} sx={{ p: 3, bgcolor: '#F8FAFC' }}>
        <Button onClick={onCancel} sx={{ fontWeight: 700, color: '#64748B', textTransform: 'none' }}>{t('admin.cancel')}</Button>
        <Button 
          variant="contained" 
          onClick={onSubmit} 
          disabled={isUpdating} 
        ...
        >
          {isUpdating ? t('admin.updating') : t('admin.saveChanges')}
        </Button>
      </Stack>
    </Paper>
  );
