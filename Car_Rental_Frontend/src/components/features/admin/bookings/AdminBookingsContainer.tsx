...
export const AdminBookingsContainer = ({ viewFilter }: { viewFilter?: ViewFilter }) => {
  ...
  if (error) return <Typography color="error">{t('containerMessages.errorLoadingRecords')}</Typography>;
  ...
  return (
    <AdminBookingsView 
      ...
      viewLabel={viewFilter?.labelKey ? t(viewFilter.labelKey) : undefined}
      t={t}
    />
  );
};