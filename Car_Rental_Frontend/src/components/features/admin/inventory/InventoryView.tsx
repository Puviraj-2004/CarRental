...
export const InventoryView = ({
  ...
  selectedItem, setSelectedItem, handleUpdate, handleConfirmDelete, t
}: any) => {

  return (
    <Box>
      <Typography variant="h4" fontWeight={900} color="#0F172A" mb={4}>{t('admin.inventoryHub')}</Typography>
      ...
      <Paper elevation={0} sx={{ borderRadius: 4, border: '1px solid #E2E8F0', overflow: 'hidden' }}>
        <Tabs value={activeTab} onChange={(_, v) => setActiveTab(v)} sx={{ borderBottom: 1, borderColor: 'divider', px: 2, bgcolor: '#F8FAFC' }}>
          <Tab label={t('admin.carBrands')} sx={{ fontWeight: 700, textTransform: 'none' }} />
          <Tab label={t('admin.vehicleModels')} sx={{ fontWeight: 700, textTransform: 'none' }} />
        </Tabs>
          ...
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} mb={5} alignItems="center">
            {activeTab === 1 && (
              <FormControl size="small" sx={{ minWidth: 220 }}>
                <InputLabel>{t('admin.parentBrand')}</InputLabel>
                <Select value={selectedBrandId} label={t('admin.parentBrand')} onChange={(e) => setSelectedBrandId(e.target.value)}>
            ...
            <TextField 
              ...
              label={activeTab === 0 ? t('admin.newBrandName') : t('admin.newModelName')} 
          ...
          <TableContainer>
            <Table>
              <TableHead sx={{ bgcolor: '#F8FAFC' }}>
                <TableRow>
                  <TableCell sx={{ fontWeight: 800, color: '#64748B' }}>{t('admin.entryName')}</TableCell>
      ...
      <Dialog open={editOpen} onClose={() => setEditOpen(false)} fullWidth maxWidth="xs" PaperProps={{ sx: { borderRadius: 3 } }}>
        <DialogTitle sx={{ fontWeight: 800, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          {t('admin.editEntry')} <IconButton onClick={() => setEditOpen(false)}><Close /></IconButton>
        </DialogTitle>
        <DialogContent>
          <TextField autoFocus margin="dense" label={t('admin.updatedName')} fullWidth value={selectedItem?.name || ''} onChange={(e) => setSelectedItem((p: any) => ({ ...p, name: e.target.value }))} sx={{ mt: 1 }} />
        </DialogContent>
        <DialogActions sx={{ p: 3 }}>
          <Button onClick={() => setEditOpen(false)} sx={{ fontWeight: 700, color: '#64748B' }}>{t('admin.cancel')}</Button>
          <Button onClick={handleUpdate} variant="contained" sx={{ bgcolor: '#0F172A', fontWeight: 700 }}>{t('admin.saveChanges')}</Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={deleteOpen} onClose={() => setDeleteOpen(false)} fullWidth maxWidth="xs" PaperProps={{ sx: { borderRadius: 4 } }}>
        <Box sx={{ p: 4, textAlign: 'center' }}>
          ...
          <Typography variant="h6" fontWeight={800}>{t('admin.confirmDeletion')}</Typography>
          <Typography variant="body2" color="text.secondary" sx={{ mt: 1, mb: 4 }}>
            {t('admin.areYouSureDelete')} <b>{selectedItem?.name}</b>? {t('admin.cannotBeUndone')}
          </Typography>
          <Stack direction="row" spacing={2}>
            <Button fullWidth variant="outlined" onClick={() => setDeleteOpen(false)} sx={{ borderRadius: 2, fontWeight: 700, color: '#64748B' }}>{t('admin.keepIt')}</Button>
            <Button fullWidth variant="contained" color="error" onClick={handleConfirmDelete} sx={{ borderRadius: 2, fontWeight: 700 }}>{t('admin.yesDelete')}</Button>
          </Stack>
        </Box>
      </Dialog>
    </Box>
