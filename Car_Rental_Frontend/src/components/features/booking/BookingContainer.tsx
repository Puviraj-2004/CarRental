...
export const BookingContainer = () => {
  ...
  const [walkInRequiresDocs, setWalkInRequiresDocs] = useState(false);
  ...
  useEffect(() => {
    ...
    if (pickupDate < today) {
      error = t('validation.pickupDateFuture');
    } else if (isToday(pickupDate) && isBefore(start, minPickupTime)) {
      error = t('validation.pickupTimeMinHour');
    } else if (end <= start) {
      error = t('validation.returnAfterPickup');
    } else {
      const durationHours = (end.getTime() - start.getTime()) / 36e5;
      if (durationHours < 2) {
        error = t('validation.minDuration');
      }
    }
  ...
  useEffect(() => {
    ...
    const shouldRequireDocs = isAdmin && isWalkIn && bookingType !== 'REPLACEMENT';
    setWalkInRequiresDocs(shouldRequireDocs);
  }, [userData?.me?.role, isWalkIn, bookingType]);
  ...
  useEffect(() => {
    if (!emailVerificationPopup) return;

    const expiresAt = confirmedBookingData?.verification?.expiresAt;
    if (!expiresAt) return;
    ...
    setVerificationTimer(formatRemaining(expiresAtMs - Date.now()));
    ...
    const interval = setInterval(() => {
      ...
      setVerificationTimer(formatRemaining(diff));
    }, 1000);
    ...
    try {
      ...
      if (isAdmin && isWalkIn) {
        if (!guestName.trim() || !guestPhone.trim()) {
          alert(t('validation.guestNamePhoneRequired'));
          return;
        }
      }
      ...
      if (isEditMode && bookingId) {
        ...
        // CREATE NEW BOOKING
        const { data: bRes } = await createBooking({
          variables: {
            input: {
              carId: targetCarId,
              ...
              bookingType: isAdmin ? bookingType : 'RENTAL',
              isWalkIn: isAdmin ? isWalkIn : false,
              guestName: isWalkIn ? guestName : undefined,
              guestPhone: isWalkIn ? guestPhone : undefined,
              guestEmail: isWalkIn ? guestEmail : undefined,
            },
          },
        });
      ...
      const { getSafeErrorMessage } = await import('@/lib/errorUtils');
      alert(getSafeErrorMessage(e, t('errors.generic')));
    }
  };
    ...
    return (
      <Container maxWidth="sm" sx={{ py: 10, textAlign: 'center' }}>
        <Alert severity="error" sx={{ mb: 3 }}>
          {bookingError?.message || carError?.message || t('containerMessages.failedLoadDetails')}
        </Alert>
        <Button variant="contained" onClick={() => router.push('/cars')}>{t('booking.backToFleet')}</Button>
      </Container>
    );
  }
  ...
  if (status === 'loading' || bookingLoading || carLoading) {
    return (
      <Box sx={{ minHeight: '80vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
        ...
        <Typography variant="body2" color="text.secondary">{t('containerMessages.fetchingSecureData')}</Typography>
      </Box>
    );
  }
  ...
  return (
    <BookingView 
      ...
      walkInRequiresDocs={walkInRequiresDocs}
