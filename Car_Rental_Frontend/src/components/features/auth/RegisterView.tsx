      ...
      <Grid container sx={{ height: '100%' }}>
          ...
          <Box sx={{ 
            ...
            <Box sx={{ mb: 2 }}>
              <Typography variant="h1" fontWeight={900} sx={{ color: 'white', mb: 1, letterSpacing: '-1.5px', fontSize: { md: '2.5rem', lg: '3.5rem' }, lineHeight: 1.1 }}>
                {t('auth.unlockFleet').split('.')[0]} <br /> {t('auth.unlockFleet').includes('.') ? t('auth.unlockFleet').split('.')[1] + '.' : ''}
              </Typography>
              <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.7)', fontWeight: 300, maxWidth: '350px' }}>
                {t('auth.joinCommunity')}
              </Typography>
            </Box>
            <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.3)', letterSpacing: '1px' }}>{settings?.companyName} © {new Date().getFullYear()}</Typography>
          </Box>
        </Grid>
        ...
        <Grid item xs={12} md={6} lg={5} sx={{ 
          ...
          <Container maxWidth="xs" sx={{ px: { xs: 0, sm: 2 } }}>
            <Box sx={{ mb: 2 }}>
              <Typography variant="h6" fontWeight={800} color={COLORS.textPrimary} sx={{ lineHeight: 1.2 }}>{t('auth.registerTitle')}</Typography>
              <Typography variant="caption" color={COLORS.textSub}>{t('auth.fillDetails')}</Typography>
            </Box>
            ...
            <form onSubmit={onSubmit} autoComplete="off">
              <Stack spacing={1.2}>
                ...
                <Grid container spacing={1.2}>
                  <Grid item xs={6}>
                    <Typography variant="caption" fontWeight={700} sx={{ mb: 0.3, display: 'block', color: COLORS.textPrimary }}>{t('auth.fullName').toUpperCase()}</Typography>
                    <TextField fullWidth placeholder={t('auth.placeholderName')} sx={inputStyles} value={formData.fullName}
                      autoComplete="off"
                  ...
                  <Grid item xs={6}>
                    <Typography variant="caption" fontWeight={700} sx={{ mb: 0.3, display: 'block', color: COLORS.textPrimary }}>{t('auth.phone').toUpperCase()}</Typography>
                    <TextField fullWidth placeholder={t('auth.placeholderPhone')} sx={inputStyles} value={formData.phoneNumber}
                      autoComplete="off"
                      name="register-phone"
                ...
                <Box>
                  <Typography variant="caption" fontWeight={700} sx={{ mb: 0.3, display: 'block', color: COLORS.textPrimary }}>{t('auth.email').toUpperCase()}</Typography>
                  <TextField fullWidth placeholder={t('auth.placeholderEmail')} sx={inputStyles} value={formData.email} error={!!fieldErrors?.email} helperText={fieldErrors?.email}
                    autoComplete="off"
                    name="register-email"
                ...
                <Box>
                  <Typography variant="caption" fontWeight={700} sx={{ mb: 0.3, display: 'block', color: COLORS.textPrimary }}>{t('auth.password').toUpperCase()}</Typography>
                  <TextField fullWidth placeholder={t('auth.createPassword')} type={showPassword ? 'text' : 'password'} sx={inputStyles} value={formData.password}
                ...
                <Box>
                  <Typography variant="caption" fontWeight={700} sx={{ mb: 0.3, display: 'block', color: COLORS.textPrimary }}>{t('auth.confirmPassword').toUpperCase()}</Typography>
                  <TextField fullWidth placeholder={t('auth.repeatPassword')} type={showPassword ? 'text' : 'password'} sx={inputStyles} value={formData.confirmPassword}
                    autoComplete="new-password"
                ...
                <FormControlLabel
                  ...
                  label={<Typography variant="caption" color={COLORS.textSub}>{t('auth.agreeTerms')} <Link href="#" underline="none" fontWeight={700} color={COLORS.accent}>{t('auth.termsPrivacy')}</Link></Typography>}
                />
                ...
                <Button 
                  ...
                  {loading ? <CircularProgress size={18} color="inherit" /> : t('auth.registerTitle')}
                </Button>

                <Typography variant="caption" textAlign="center" sx={{ color: COLORS.textSub }}>
                  {t('auth.hasAccount')} <Link href="/login" sx={{ color: COLORS.accent, fontWeight: 700, textDecoration: 'none' }}>{t('auth.signIn')}</Link>
                </Typography>
              </Stack>
            </form>
          </Container>
