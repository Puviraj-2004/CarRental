...
export const RegisterContainer = () => {
  ...
  const validatePassword = (password: string) => {
    ...
    if (strength < 75) {
      setFieldErrors(prev => ({ ...prev, password: t('validation.passwordRequirements') }));
    } else {
  ...
  const handlePasswordChange = (password: string) => {
    ...
    if (formData.confirmPassword && formData.confirmPassword !== password) {
      setFieldErrors(prev => ({ ...prev, confirmPassword: t('validation.passwordsDoNotMatch') }));
    } else {
  ...
  const validateEmail = (email: string) => {
    ...
    if (!emailRegex.test(email)) {
      setFieldErrors(prev => ({ ...prev, email: t('validation.enterValidEmail') }));
    } else {
    ...
    emailCheckTimer.current = setTimeout(async () => {
      ...
      if (available === false) {
        setFieldErrors(prev => ({ ...prev, email: t('validation.emailInUse') }));
      } else if (available === true) {
        // Only clear if previously set as taken
        setFieldErrors(prev => ({ ...prev, email: prev.email === t('validation.emailInUse') ? undefined : prev.email }));
      }
    }, 500);
  };
  ...
  const validatePhone = (phone: string) => {
    ...
    if (!/^\+?[1-9]\d{6,14}$/.test(clean) || digits.length < 7 || digits.length > 15) {
      setFieldErrors(prev => ({ ...prev, phone: t('validation.enterValidPhone') }));
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (fieldErrors.email || fieldErrors.phone || fieldErrors.password || fieldErrors.confirmPassword) {
      setError(t('validation.fixHighlightedErrors'));
      return;
    }
    ...
    try {
      ...
      if (/exists|already/i.test(msg)) {
        setError(t('validation.accountExists'));
      } else {
        setError(msg);
      }
    }
