import { isAuthenticated, isAdmin, isOwnerOrAdmin } from '../../utils/authguard';
import { bookingService } from '../../services/bookingService';
import { AppError, ErrorCode } from '../../errors/AppError';

export const bookingResolvers = {
  Query: {
    bookings: async (_: any, { pagination }: { pagination?: any }, context: any) => {
      isAdmin(context);
      return await bookingService.getAllBookings(pagination);
    },

    myBookings: async (_: any, __: any, context: any) => {
      isAuthenticated(context);
      return await bookingService.getBookingsByUserId(context.userId);
    },

    checkCarAvailability: async (_: any, { carId, startDate, endDate, excludeBookingId }: any) => {
      return await bookingService.checkAvailability(carId, startDate, endDate, excludeBookingId);
    },

    booking: async (_: any, { id }: { id: string }, context: any) => {
      isAuthenticated(context);
      const booking = await bookingService.getBookingById(id);
      if (!booking) return null;
      // For walk-in bookings (userId is null), only admins can access
      if (booking.userId === null) {
        isAdmin(context);
      } else {
        isOwnerOrAdmin(context, booking.userId);
      }
      return booking;
    },

    bookingByToken: async (_: any, { token }: { token: string }) => {
      return await bookingService.getBookingByToken(token);
    },

    userBookings: async (_: any, { userId }: { userId: string }, context: any) => {
      isAdmin(context);
      return await bookingService.getBookingsByUserIdBasic(userId);
    },

    carBookings: async (_: any, { carId }: { carId: string }, context: any) => {
      isAdmin(context);
      return await bookingService.getBookingsByCarId(carId);
    }
  },

  Mutation: {
    createBooking: async (_: any, { input }: { input: any }, context: any) => {
      isAuthenticated(context);
      return await bookingService.createBooking(context.userId, context.role, input);
    },

    confirmReservation: async (_: any, { id }: { id: string }, context: any) => {
      isAuthenticated(context);
      const bookingAuth = await bookingService.getBookingForAuth(id);
      if (!bookingAuth) {
        throw new AppError('Booking not found', ErrorCode.NOT_FOUND);
      }

      // For walk-in bookings (userId is null), only admins can confirm
      if (bookingAuth.userId === null) {
        isAdmin(context);
      } else {
        isOwnerOrAdmin(context, bookingAuth.userId);
      }

      return await bookingService.confirmReservation(id, context.userId, context.role);
    },

    startTrip: async (_: any, { bookingId, startOdometer, pickupNotes }: { bookingId: string; startOdometer?: number; pickupNotes?: string }, context: any) => {
      isAuthenticated(context);
      return await bookingService.startTrip(bookingId, startOdometer, pickupNotes);
    },

    completeTrip: async (_: any, { bookingId }: { bookingId: string }, context: any) => {
      isAdmin(context);
      return await bookingService.completeTrip(bookingId);
    },

    finishCarMaintenance: async (_: any, { carId }: { carId: string }, context: any) => {
      isAdmin(context);
      return await bookingService.finishCarMaintenance(carId);
    },

    updateBookingStatus: async (_: any, { id, status }: any, context: any) => {
      return await bookingService.updateBookingStatus(id, status, context.userId, context.role);
    },

    cancelBooking: async (_: any, { id, reason }: { id: string; reason?: string }, context: any) => {
      isAuthenticated(context);
      await bookingService.cancelBooking(id, context.userId, context.role, reason);
      return true;
    },

    deleteBooking: async (_: any, { id }: { id: string }, context: any) => {
      isAdmin(context);
      await bookingService.deleteBooking(id);
      return true;
    },

    updateBooking: async (_: any, { id, input }: { id: string, input: any }, context: any) => {
      isAuthenticated(context);
      return await bookingService.updateBooking(id, input, context.userId, context.role);
    }
  },

  // ─── Field-level resolvers (DataLoader-backed, prevents N+1) ────────────
  Booking: {
    user: async (parent: any, _args: any, context: any) => {
      if (parent.user) return parent.user;
      if (parent.userId && context.loaders) {
        return context.loaders.userLoader.load(parent.userId);
      }
      return null;
    },
    car: async (parent: any, _args: any, context: any) => {
      if (parent.car) return parent.car;
      if (parent.carId && context.loaders) {
        return context.loaders.carLoader.load(parent.carId);
      }
      return null;
    },
    payment: async (parent: any, _args: any, context: any) => {
      if (parent.payment !== undefined) return parent.payment;
      if (context.loaders) {
        return context.loaders.paymentByBookingLoader.load(parent.id);
      }
      return null;
    },
    verification: async (parent: any, _args: any, context: any) => {
      if (parent.verification !== undefined) return parent.verification;
      if (context.loaders) {
        return context.loaders.bookingVerificationByBookingLoader.load(parent.id);
      }
      return null;
    },
    documentVerification: async (parent: any, _args: any, context: any) => {
      if (parent.documentVerification !== undefined) return parent.documentVerification;
      if (context.loaders) {
        return context.loaders.docVerificationByBookingLoader.load(parent.id);
      }
      return null;
    }
  }
};