...
import { NormalizedPagination, buildSearchFilter, buildPaginatedResult, PaginatedResult } from '../utils/pagination';
...
export class BookingRepository {
   ...
   * Searches across: booking ID, user name, car plate number, guest name/email.
   */
  async findPaginated(
    pagination: NormalizedPagination,
    include: any = BOOKING_INCLUDES.admin,
    extraWhere: any = {}
  ): Promise<PaginatedResult<any>> {
    const where: any = { ...extraWhere, status: { not: BookingStatus.DRAFT } };

    // Server-side search
    if (pagination.search) {
      const searchFilter = buildSearchFilter(pagination.search, [
        'user.fullName',
        'car.plateNumber',
        'guestName',
        'guestEmail',
      ...
      if (searchFilter) {
        // Also search by booking ID prefix
        where.AND = [
          {
            OR: [
