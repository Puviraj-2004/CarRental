...
export const uploadToCloudinary = async (
  ...
  if (!cloudinaryReady) {
    ...
    if (Buffer.isBuffer(fileInput)) {
      ...
      throw new AppError('Unsupported file input type', ErrorCode.UPLOAD_ERROR);
    }

    const publicPath = path.join('car_rental_industrial', folder, safeName).replace(/\\/g, '/');
    const backendBaseUrl =
      normalizeBackendBaseUrl(process.env.BACKEND_URL) || `http://localhost:${process.env.PORT || 4000}`;
    const secureUrl = `${backendBaseUrl}/uploads/${publicPath}`;
  ...
  const attemptUpload = async (attempt: number): Promise<any> => {
    return new Promise((resolve, reject) => {
      ...
      if (Buffer.isBuffer(fileInput)) {
        ...
        reject(new AppError('Unsupported file input type', ErrorCode.UPLOAD_ERROR));
      }
    }).catch(async (error) => {
