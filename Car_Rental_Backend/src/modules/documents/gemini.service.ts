...
export class OCRService {
  ...
  constructor() {
    ...
    if (!apiKey) {
      throw new AppError('GEMINI_API_KEY is missing. Set it in your environment variables.', ErrorCode.CONFIGURATION_ERROR);
    }
  ...
  async extractDocumentData(
    ...
    securityLogger.info('OCR request started', { documentType, side });
    try {
  ...
  private pickHighestCategory(categories: string[]): string {
    const rank: Record<string, number> = {
      AM: 0, A1: 10, A2: 11, A: 12,
      B1: 20, B: 21, BE: 22,
      C1: 30, C: 31, C1E: 32, CE: 33,
      D1: 40, D: 41, D1E: 42, DE: 43,
    };
  ...
  private combineNameFields(data: Partial<ExtractedDocumentData>): string {
