...
class CleanupService {
  ...
  async cleanupOldCompletedBookings(daysOld: number = 90): Promise<{ deletedCount: number }> {
    try {
      ...
      throw new AppError('Failed to cleanup old completed bookings', ErrorCode.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async getCleanupStats() {
    try {
      ...
      throw new AppError('Failed to get cleanup statistics', ErrorCode.INTERNAL_SERVER_ERROR);
    }
  }
}
