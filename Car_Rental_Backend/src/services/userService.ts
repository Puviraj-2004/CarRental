...
import { logSecurityEvent, securityLogger } from '../utils/logger';
import { validatePassword } from '../utils/validation';
import { validateFileMime, validateFileExtension } from '../utils/fileValidation';
import { BookingStatus as PrismaBookingStatus, VerificationStatus } from '@prisma/client';
...
export class UserService {
  private async uploadIfPresent(file: any, folder: string): Promise<string | undefined> {
    ...
    try {
      ...
      if (!createReadStream) {
        securityLogger.error('Upload: no createReadStream found in file object', { folder });
        return undefined;
      }

      // Validate file type before uploading
      validateFileMime(mimetype, 'verification_document');
      validateFileExtension(filename, 'verification_document');
      
      const result = await uploadToCloudinary(createReadStream(), folder);
      return result.secure_url;
    } catch (e) {
      securityLogger.error('Upload failed', { folder, error: e instanceof Error ? e.message : String(e) });
      return undefined;
    }
  }
  async register(input: RegisterInput) {
    ...
    await storePendingRegistration(email, fullName || '', hashedPassword, phoneNumber);
  ...
  // 1. Validate booking exists first
    ...
    if (!bookingExists) {
      throw new AppError(`Booking with ID ${bookingId} not found`, ErrorCode.NOT_FOUND);
    }
    ...
    const verificationData = {
      ...
      licenseCategories: convertedLicenseCategories, // 
    ...
    if (hasAllDocs && input.bookingToken) {
      ...
      if (bVerif) {
        ...
        await bookingRepository.updateBookingStatus(bVerif.bookingId, PrismaBookingStatus.VERIFIED);
      }
    }
  ...
  async verifyDocument(bookingId: string, status: DocumentVerificationStatus) {
    ...
    if (!booking) return verification;

    if (status === VerificationStatus.APPROVED) {
      const terminalStatuses = ['CANCELLED', 'REJECTED', 'EXPIRED', 'COMPLETED', 'ONGOING', 'CONFIRMED'] as const;

      // Do NOT downgrade paid/confirmed (or other terminal) bookings back to VERIFIED
      if (!(terminalStatuses as readonly string[]).includes(booking.status as string)) {
        await prisma.booking.update({
          where: { id: bookingId },
          data: { status: PrismaBookingStatus.VERIFIED }
        });
      }
      ...
      // Avoid auto-cancelling paid/confirmed bookings here; handle cancellation separately if needed
      if (!(terminalStatuses as readonly string[]).includes(booking.status as string)) {
        await prisma.booking.update({
          where: { id: bookingId },
      ...
      if (payment && payment.status === 'SUCCEEDED') {
        ...
        try {
          ...
          securityLogger.info('Refund initiated for document rejection', { bookingId, paymentId: payment.id });
        } catch (refundError) {
          const msg = typeof refundError === 'object' && refundError && 'message' in refundError ? (refundError as any).message : String(refundError);
          securityLogger.error('Refund failed', { bookingId, paymentId: payment.id, error: msg });
        }
      }
    }
    return verification;
  ...
  async processOCR(file: FileUpload, documentType: string, side: string) {
    ...
    // Validate declared MIME + extension before processing
    validateFileMime(mimetype, 'ocr_document');
    validateFileExtension(filename, 'ocr_document');
    ...
    validateFileMime(mimetype, 'ocr_document', buffer);
  ...
  async getAllUsers(pagination?: PaginationInput) {
    const normalized = normalizePagination(pagination);
    return await userRepository.findPaginated(normalized);
  }
