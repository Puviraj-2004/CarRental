...
import { Prisma, PaymentStatus, VerificationStatus } from '@prisma/client';
...
export class BookingService {
  ...
  async createBooking(userId: string, role: string, input: CreateBookingInput) {
    ...
    const effectiveBookingType = (input.bookingType || 'RENTAL') as any;
    const walkInRequiresDocs = isWalkIn && effectiveBookingType !== 'REPLACEMENT';

    const verificationToken = walkInRequiresDocs ? crypto.randomBytes(32).toString('hex') : null;
    const verificationExpiresAt = walkInRequiresDocs ? new Date(Date.now() + 60 * 60 * 1000) : null;
    ...
    if (input.bookingType === 'REPLACEMENT') {
      ...
      logger.info('Courtesy booking created — document verification skipped', { bookingType: 'REPLACEMENT', input });
    }
    ...
    const bookingData: Prisma.BookingCreateInput = {
      ...
      status: isWalkIn ? (walkInRequiresDocs ? BookingStatus.PENDING : BookingStatus.VERIFIED) : BookingStatus.DRAFT,
  ...
  async confirmReservation(id: string, userId: string, role?: string) {
    const booking = await bookingRepository.findUnique(id, BOOKING_INCLUDES.detailed);
    if (!booking) throw new AppError('Booking not found', ErrorCode.NOT_FOUND);

    // Walk-in bookings are admin-created and may not have a userId.
    // For these, confirmation is either not required (already PENDING/VERIFIED)
    // or must be performed only by an admin.
    if ((booking as any).isWalkIn) {
      if (role !== 'ADMIN') {
        throw new AppError('Only admins can confirm walk-in bookings', ErrorCode.FORBIDDEN);
      }
      return booking;
    }

    // Normal user bookings must belong to the user and be in DRAFT status.
    if (booking.userId !== userId) throw new AppError('Booking not found', ErrorCode.NOT_FOUND);
  ...
  async startTrip(bookingId: string, startOdometer?: number, pickupNotes?: string) {
    ...
    if (booking.bookingType === 'REPLACEMENT') {
      // Audit log: Skipping document verification for courtesy booking
      logger.info('Courtesy booking startTrip — document verification skipped', { bookingType: 'REPLACEMENT', bookingId });
  ...
  async getAllBookings(pagination?: PaginationInput) {
    if (pagination) {
      const normalized = normalizePagination(pagination);
      return await bookingRepository.findPaginated(normalized);
    }
    // Fallback: unpaginated (default page 1, size 20)
  ...
  async cancelBooking(id: string, userId: string, role: string, _reason?: string) {
    ...
    if (booking.status === BookingStatus.COMPLETED || 
      ...
      throw new AppError('Cannot cancel a completed or cancelled booking', ErrorCode.BAD_USER_INPUT);
    }
    ...
    if (payment && payment.status === PaymentStatus.SUCCEEDED) {
      try {
        ...
        logger.info('Refund initiated for cancelled booking', { bookingId: id, paymentId: payment.id });
      } catch (refundError: any) {
        // Log error but still proceed with cancellation
        logger.error('Refund failed during booking cancellation', { bookingId: id, paymentId: payment.id, error: refundError.message });
    ...
    // User cancel = CANCELLED, Admin cancel = CANCELLED
